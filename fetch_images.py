...
def _get_field_visual_guide(clean_title, training_goal=""):
    ...
    # 영상/편집/촬영/유튜브/크리에이터
    # v7.1: 5분기 우선순위 (구체적 컨텍스트 → 보편 키워드 순서)
    # ① 유튜브/크리에이터 + AI → AI 콘텐츠 제작 데스크 (가장 구체적)
    #   · 사용자 trainingGoal에 "편집" 같은 보편 키워드가 섞여도 우선
    # ② 편집 키워드 (위 매칭 안 됨) → 영상편집 워크스테이션 (전통 편집 과정)
    # ③ 유튜브/크리에이터 또는 AI (편집 없음) → AI 데스크
    # ④ 전통 촬영 (촬영 키워드만) → 촬영 스튜디오
    # ⑤ 디폴트 → 워크스테이션
    if has_video:
        ...
        ai_desk_guide = {
            "subject": (
                "A modern AI-powered content creator's desk, fully equipped "
                "but currently empty. The scene shows: a wide primary "
                "monitor displaying an AI content generation interface, a "
                "condenser microphone mounted on a small desk arm for AI "
                "voiceover recording, a pair of studio over-ear headphones "
                "beside the keyboard, a tablet propped on a stand showing "
                "thumbnail design layouts, and a small closed notebook. "
                "Soft warm desk lighting, modern minimalist aesthetic. "
                "The creative tools — monitor, microphone, headphones, "
                "tablet — are the central focal points, since the visual "
                "content is generated by AI rather than filmed with a "
                "traditional camera."
            ),
            "human_policy": (
                "no people visible in the frame — the creator's desk is "
                "fully equipped but unoccupied"
            ),
            "monitor_content": (
                "The primary monitor displays a generative AI content "
                "creation interface with a dark modern UI. Layout: LEFT "
                "1/4 of the screen is a vertical sidebar with stacked "
                "input controls — short horizontal colored bars and "
                "toggle pill shapes representing prompt and parameter "
                "fields. RIGHT 3/4 of the screen is dominated by a 3x3 "
                "grid of generated video thumbnail tiles, each tile "
                "showing a different AI-generated cinematic landscape "
                "scene (Jeju coastline, oreum hills, ocean waves, sunset, "
                "forest, sky). BOTTOM strip across the full width: a "
                "small horizontal row with 4-5 short colored segments "
                "representing a brief timeline. Pure dark modern UI "
                "aesthetic. NO text, NO file names, NO panel labels."
            ),
        }

        # ① 유튜브/크리에이터 + AI → AI 데스크 (편집 키워드 무관)
        if has_creator_modern and has_ai:
            return ai_desk_guide
...
def _build_image_prompt(course_data):
    ...
    v7.1 변경점 (편집 키워드가 trainingGoal에 섞여도 AI 데스크 우선):
      · 영상 분야 5분기 우선순위
      · ① 유튜브/크리에이터 + AI → AI 데스크 (편집 키워드 무관)
        실제 사용자 케이스 trainingGoal에 "Vrew·CapCut 편집"처럼 보편 키워드가
        섞여도 우선 매칭
      · ② 편집 → 워크스테이션
      · ③ 유튜브/크리에이터 OR AI (편집 없음) → AI 데스크
      · ④ 전통 촬영 → 촬영 스튜디오
      · ⑤ 디폴트 → 워크스테이션

    v7 유지: AI 콘텐츠 제작 데스크 (Google AI Studio · ElevenLabs · Vrew · CapCut ·
             미리캔버스 등 AI 도구 환경 부합)
