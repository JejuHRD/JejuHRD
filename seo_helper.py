...
TITLE_FIELD_KEYWORDS = {
    ...
    "멀티미디어": ["멀티미디어"],
    "콘텐츠": ["콘텐츠", "크리에이터"],
    # 마케팅복합: 실제 인자위 과정명이 "AI 마케팅 자동화", "AI 유튜브 크리에이터"처럼
    #   'AI'로 시작하거나 영상/콘텐츠 키워드를 포함해, 아래 일반 "마케팅" 항목만으로는
    #   'AI'·'영상'·'콘텐츠' 라벨에 먼저 흡수됨. 복합 키워드를 우선순위 앞단에서 선점시켜
    #   마케팅 리서치 데이터(AI 마케팅 자동화 + AI 유튜브 크리에이터)에 도달하게 함.
    #   → detect 시 이 라벨이 잡히면 매핑 테이블에서 "마케팅"으로 환원(_PRIORITY_LABEL_TO_KEY).
    "마케팅복합": ["AI마케팅", "마케팅자동화", "퍼포먼스마케팅", "디지털마케팅",
               "AI유튜브", "유튜브크리에이터", "유튜브마케팅", "SNS마케팅", "콘텐츠마케팅"],
    "마케팅": ["마케팅", "퍼포먼스", "광고"],
    "데이터": ["데이터", "빅데이터", "분석", "시각화"],
    "코딩": ["코딩", "프로그래밍", "파이썬", "개발", "자바", "웹개발"],
    "물류/운송": ["지게차", "물류", "포클리프트", "건설기계조종", "운전기능사", "하역", "창고관리"],
    "조경": ["조경", "조경기능사", "정원", "수목", "식재", "가드닝", "그린키퍼", "녹지", "식물관리"],
    "에너지/시설관리": ["에너지관리", "에너지 관리", "기계설비유지관리", "기계설비", "보일러", "냉동공조", "시설관리", "HVAC", "냉난방", "열관리", "배관"],
}

# 감지용 복합 라벨 → 실제 리서치 JSON 키 환원 테이블
#   detect_course_field가 우선순위 단계에서 반환하는 '가상 라벨'을
#   field_research.json의 실제 키로 되돌린다.
#   (예: "마케팅복합" 라벨로 잡아 AI/영상 흡수를 막되, 데이터는 "마케팅" 키에서 조회)
_PRIORITY_LABEL_TO_KEY = {
    "마케팅복합": "마케팅",
}
...
def detect_course_field(title, ncs_cd=None):
    ...
    #   · "제과제빵"·"관광데이터"·"AI커머스"·"디지털콘텐츠"·"마케팅복합"은 "AI"보다 앞
    #     (제목에 'AI 활용'이 붙는 과정이 전부 AI 분야로 흡수되는 것을 방지)
    #   · "마케팅복합"은 "영상"·"콘텐츠"보다도 앞 — "AI 유튜브 크리에이터",
    #     "유튜브 크리에이터 마케팅" 등이 영상/콘텐츠로 새는 것을 방지
    _priority_fields = [
        "드론정비", "드론",
        "제과제빵", "관광데이터", "AI커머스", "디지털콘텐츠", "마케팅복합",
    ...
    for field in _priority_fields:
        if field in TITLE_FIELD_KEYWORDS:
            for kw in TITLE_FIELD_KEYWORDS[field]:
                if _match(kw):
                    # 복합 라벨은 실제 리서치 JSON 키로 환원
                    return _PRIORITY_LABEL_TO_KEY.get(field, field)
    # 2단계: NCS 코드 매핑
...
EMPATHY_INTROS = {
    ...
    "마케팅": [
        "AI를 활용하는 마케터는 그렇지 않은 마케터보다 임금이 56% 높고 생산성은 25.6% 앞서요. 마케팅 직무의 65%가 AI 대체 위험에 놓인 지금, 'AI를 쓰는 마케터'가 될 것인지 선택해야 할 때예요.",
        "한국 디지털 광고 시장이 10조 원을 돌파하며 전체 광고의 60%를 넘어섰어요. 네이버·카카오·메타·구글 모두 AI 자동화로 전면 전환한 지금, 새로운 마케팅 전략을 모르면 광고비만 날리게 돼요.",
        "제주 관광사업체의 91%가 직원 4명 이하 영세 사업장이에요. 비짓제주 이용자가 720만 명을 돌파하고 관광 의사결정이 완전히 디지털로 옮겨간 시대, 마케팅전략기획 역량이 곧 매출이에요.",
        "제주도가 디지털전환에 1,933억 원을 투자하고 있지만, 이를 활용할 마케팅 인재가 부족해요. 그로스 마케터 평균 연봉 6,672만 원, 기업 69.2%가 AI 역량을 보는 시대의 기회를 잡으세요.",
        "K-디지털 트레이닝 전액 국비지원은 2025년 개강분이 마지막이에요. ChatGPT부터 GA4, 퍼포먼스 마케팅까지 AI 시대 마케팅 전략을 무료로 배울 수 있는 마지막 기회, 놓치지 마세요.",
    ],
    "산업안전": [
